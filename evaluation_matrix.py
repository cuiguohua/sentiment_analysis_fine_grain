...
def compute_confuse_matrix(y_targetlabel_list_single, y_logit_array_single, label_dict, name='default'):
    ...
    y_predict_labels = [i for i in range(len(y_logit_array_single)) if
                        y_logit_array_single[i] >= 0.50]  # TODO 0.5PW e.g.[2,12,13,10]
    ...
    if random.choice([x for x in range(random_number)]) == 1: print(name + ".y_target_labels:", y_target_labels,
                                                                    ";y_predict_labels:",
                                                                    y_predict_labels)  # debug purpose
...
def compute_penalty_score_batch(target_deaths, predict_deaths, target_lifeimprisons, predict_lifeimprisons,
                                target_imprsions, predict_imprisons):
    """
    ...
    for i in range(length):
        score = compute_penalty_score(target_deaths[i], predict_deaths[i], target_lifeimprisons[i],
                                      predict_lifeimprisons[i], target_imprsions[i], predict_imprisons[i])
        score_total = score_total + score
    score_batch = score_total / float(length)
    return score_batch


def compute_penalty_score(target_death, predict_death, target_lifeimprison, predict_lifeimprison, target_imprsion,
                          predict_imprison):
    """
...
def compute_death_lifeimprisonment_score(target, predict):
    ...
    if random.choice([x for x in range(random_number)]) == 1: print("death_lifeimprisonment_score.target:", target,
                                                                    ";predict:", predict)
    if target == predict:
        score = 1.0
...
def compute_imprisonment_score(target_value, predict_value):
    ...
    :return: score: a scalar
    """
    if random.choice([x for x in range(random_number)]) == 1: print("x.imprisonment_score.target_value:", target_value,
