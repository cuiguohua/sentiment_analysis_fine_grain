...
class BertModel:
    def __init__(self, config):
        ...
        self.input_x = tf.placeholder(tf.int32, [self.batch_size, self.sequence_length],
        ...
        if not self.is_fine_tuning:
            ...
            self.predictions_lm = tf.argmax(self.logits_lm, axis=1,
                                            name="predictions")  # shape:[None,]<---[ None,num_classes]
    ...
    def inference_lm(self):
        ...
        token_embeddings = tf.nn.embedding_lookup(self.embedding,
                                                  self.x_mask_lm)  # [batch_size,sequence_length,embed_size]
        self.input_representation_lm = tf.add(tf.add(token_embeddings, self.segment_embeddings_lm),
                                              self.position_embeddings_lm)  # [batch_size,sequence_length,embed_size]

        # 2. repeat Nx times of building block( multi-head attention followed by Add & Norm; feed forward followed by Add & Norm)
        encoder_class = Encoder(self.d_model, self.d_k, self.d_v, self.sequence_length_lm, self.h, self.batch_size,
                                self.num_layer, self.input_representation_lm,
                                self.input_representation_lm, dropout_keep_prob=self.dropout_keep_prob,
    ...
    def inference(self):
        ...
        # 1. input representation(input embedding, positional encoding, segment encoding)
        token_embeddings = tf.nn.embedding_lookup(self.embedding,
                                                  self.input_x)  # [batch_size,sequence_length,embed_size]
    ...
    def loss_lm(self, l2_lambda=0.0001 * 3):
        ...
        losses = tf.nn.softmax_cross_entropy_with_logits_v2(labels=y_mask_lm_onehot,
                                                            logits=self.logits_lm)  # [batch_size,num_classes]
        print("#loss_lm.losses:", losses)
        lm_loss = tf.reduce_mean(losses)
        self.l2_loss_lm = tf.add_n(
    ...
    def loss(self, l2_lambda=0.0001 * 3):
        ...
        losses = tf.nn.sigmoid_cross_entropy_with_logits(labels=self.input_y,
                                                         logits=self.logits)  # [batch_size,num_classes]
        self.losses = tf.reduce_mean(
            (tf.reduce_sum(losses, axis=1)))  # shape=(?,)-->(). loss for all data in the batch-->single loss
    ...
    def train_lm(self):
        ...
        train_op = tf.contrib.layers.optimize_loss(self.loss_val_lm, global_step=self.global_step,
    ...
    def instantiate_weights(self):
        ...
        with tf.name_scope("embedding"):  # embedding matrix
            self.embedding = tf.get_variable("embedding", shape=[self.vocab_size, self.d_model],
                                             initializer=self.initializer)  # [vocab_size,embed_size]
            self.segment_embeddings_lm = tf.get_variable("segment_embeddings_lm", [self.d_model],
                                                         initializer=tf.constant_initializer(
                                                             1.0))  # a learned sequence embedding
            self.position_embeddings_lm = tf.get_variable("position_embeddings_lm",
                                                          [self.sequence_length_lm, self.d_model],
                                                          initializer=tf.constant_initializer(
                                                              1.0))  # sequence_length,1]
            ...
            self.position_embeddings = tf.get_variable("position_embeddings", [self.sequence_length, self.d_model],
                                                       initializer=tf.constant_initializer(
                                                           1.0))  # [sequence_length,self.d_model]
...
def train():
    ...
    with tf.Session(config=gpu_config) as sess:
        ...
        for i in range(10000):
            ...
            loss, _ = sess.run([model.loss_val, model.train_op],
                               feed_dict={model.x_mask_lm: input_x, model.y_mask_lm: input_y,
                                          model.p_mask_lm: p_mask_lm,
                                          model.dropout_keep_prob: config.dropout_keep_prob})
            print(i, "loss:", loss, "-------------------------------------------------------")
            if i == 300:
                print("label[0]:", input_y[0]);
...
def predict():
    ...
    with tf.Session(config=gpu_config) as sess:
        ...
        for i in range(100):
            ...
            # 3.run session to train the model, print some logs.
            logit, prediction = sess.run([model.logits, model.predictions], feed_dict={model.input_x: input_x,
                                                                                       model.dropout_keep_prob: config.dropout_keep_prob})
            print("target_label:", target_label, ";input_sum:", input_sum, "threshold:", threshold, ";prediction:",
                  prediction);
