...
import csv
import os
from model import bert_modeling as modeling
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
    ...
    if ex_index < 5:
        ...
        if "," in example.label:
            tf.logging.info("label: %s (id_list = %s)" % (
                str(example.label),
                str(label_id_list)))  # if label_id is a list, try print multi-hot value: label_id_list
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
                 labels, num_labels, use_one_hot_embeddings):
    """Creates a classification model."""
    print('>>>>>>>>>>', is_training, input_ids, input_mask, segment_ids,
          labels, num_labels, use_one_hot_embeddings)
    model = modeling.BertModel(
        config=bert_config,
        is_training=is_training,
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(per_example_loss, label_ids, logits):
                ...
                for j, logits in enumerate(logits_split):  #
                    ...
                    print("label_ids_split[j]:", label_ids_split[j], ";predictions:", predictions, ";label_id_:",
                          label_id_)
